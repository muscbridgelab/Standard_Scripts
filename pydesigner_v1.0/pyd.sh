...
read -p $'\033[1;36mEnter DKI file name to process. Eg. DKI_30.nii or DKI_BIPOLAR_2.5_64dir_50slices.nii.gz:\e[0m ' DKI
read -p $'\033[1;36mEnter TOPUP file name to process. Eg. DKI_30_TOPUP.nii or DKI_BIPOLAR_2.5_64dir_50slices_TOPUP.nii.gz (leave blank if subject does not have TOPUP data):\e[0m ' TOPUP

for i in $SUBJ_IDs ; do 
  echo -e "${BYellow}Beginning PyDesigner for ${i}${NC}"
  mkdir $base/03_Analysis/${i}
